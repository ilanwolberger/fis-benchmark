...
- No timestamped pre-registration for v0: the anti-cherry-pick guarantee rests on the author's assertion plus retained-problematic-repos evidence, not an independently auditable dated artifact. Lock-file pre-registration is a v1 commitment. **— STILL OPEN.** The held-out and vibe corpora were frozen before their first run, but neither was published to a dated public tag beforehand, so the same "asserted, not cryptographically established" standing applies to them too.
